    ...
    def _analyze_emotion2(self, pos,):
        ...
        for category in emotions.keys():
            ...
            for (token, tag) in pos:
                ...
                if token[:-1] == 'CLAUSE':
                    continue
                word_sets = [sent_w for sent_w in list(sentiwordnet.senti_synsets(token, pos=pos_to_wn_tag(tag[0]))) if sent_w.obj_score() < 0.75]
                print(word_sets)
...
if __name__ == '__main__':
    ...
    Every year during July the Catholic Church  has a Festival to raise Money for the Church in New York.
    Thats great and all, but it gets in my Way and causes unnecessary Amounts of Stress.
    The Problem is the Church is on the same Street as the Shop.
